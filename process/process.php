...
if ($requestMethod === 'POST' || (strpos($action, 'fetch_') === 0 && !empty($action))) {
    ...
    try {
        ...
        elseif (in_array($action, ['create_rs', 'edit_rs', 'approve_rs', 'reject_rs', 'stage_rs_materials', 'create_po', 'update_po_eta', 'mark_po_out_for_delivery', 'mark_po_delivered', 'cancel_po', 'log_viber_order_sent', 'log_po_delay', 'create_withdrawal', 'fetch_rs_data', 'fetch_rs_with_history', 'fetch_po_items', 'fetch_po_details', 'fetch_supplier_delivery_history', 'fetch_po_viber_preview', 'fetch_combined_alerts'])) {
